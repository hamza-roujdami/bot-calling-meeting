...
namespace CallingBotSample.Services.MicrosoftGraph
{
    public static class MicrosoftGraphExtensions
    {
        ...
        public static IServiceCollection AddMicrosoftGraphServices(this IServiceCollection services, Action<AzureAdOptions> azureAdOptionsAction)
        {
            ...
            TokenCredential authenticationProvider;
            
            // Use Managed Identity if no client secret is provided
            if (string.IsNullOrEmpty(options.ClientSecret))
            {
                authenticationProvider = new DefaultAzureCredential();
            }
            else
            {
                authenticationProvider = new ClientSecretCredential(options.TenantId, options.ClientId, options.ClientSecret);
            }
